...
int infoWindow(int *status){
	...
	*status = 1;
	Rectangle window = {(float) WIDTH/4, (float) HEIGHT/4, (float) WIDTH/2, (float) HEIGHT/2};
	DrawRectangle(0, 0, WIDTH, HEIGHT, Fade(RAYWHITE, 0.8f));
	int gui = GuiMessageBox(window, GuiIconText(ICON_FILETYPE_INFO, "INFO"), NULL, "Ok");

	DrawText("Purpose:", WIDTH/4 + 10, HEIGHT/4 + 40, 30, BLACK);
	DrawText(TextFormat("\tBasic RCS analizies based on Raytracing. General use for basic shapes with Wavefront\n(*.obj) graphics. More complex & in depths shapes could couse uncertainty or errors."), WIDTH/4 + 10, HEIGHT/4 + 90, 20, BLACK);
	DrawText("Supported Operating Systems:", WIDTH/4 + 10, HEIGHT/4 + 140, 30, BLACK);
	DrawText(TextFormat("\t\tLinux"), WIDTH/4 + 10, HEIGHT/4 + 190, 20, BLACK);
	DrawText(TextFormat("\t\tWindows 10/11 32bits & 64bits (older versions not tested)"), WIDTH/4 + 10, HEIGHT/4 + 240, 20, BLACK);
	// acronim
	DrawText(TextFormat("Acronym"), WIDTH/4 + 10, HEIGHT/4 + 290, 30, BLACK);

	DrawText(TextFormat("\t\tS\t-\tSuper"), WIDTH/4 + 10, HEIGHT/4 + 320, 20, BLACK);
	DrawText(TextFormat("\t\tP\t-\tPowerfull"), WIDTH/4 + 10, HEIGHT/4 + 340, 20, BLACK);
	DrawText(TextFormat("\t\tE\t-\tEngine"), WIDTH/4 + 10, HEIGHT/4 + 360, 20, BLACK);
	DrawText(TextFormat("\t\tC\t-\tComputing"), WIDTH/4 + 10, HEIGHT/4 + 380, 20, BLACK);
	DrawText(TextFormat("\t\tT\t-\tTracing"), WIDTH/4 + 10, HEIGHT/4 + 400, 20, BLACK);
	DrawText(TextFormat("\t\tR\t-\tRays"), WIDTH/4 + 10, HEIGHT/4 + 420, 20, BLACK);
	DrawText(TextFormat("\t\tO\t-\tOf"), WIDTH/4 + 10, HEIGHT/4 + 440, 20, BLACK);
	DrawText(TextFormat("\t\tN\t-\tNumerics"), WIDTH/4 + 10, HEIGHT/4 + 460, 20, BLACK);
...
int helpWindow(int *message){
	...
	// column 1
	DrawText("Controls:", WIDTH/4 + 50, HEIGHT/4 + 70, 20, BLACK);

	DrawText(TextFormat("\t\tNew Project"), WIDTH/4 + 50, HEIGHT/4 + 100, 20, BLACK);
	DrawText("<Ctrl + N>", WIDTH/4 + 300, HEIGHT/4 + 100, 20, BLACK);

	DrawText(TextFormat("\t\tSave data"), WIDTH/4 + 50, HEIGHT/4 + 130, 20, BLACK);
	DrawText("<Ctrl + S>", WIDTH/4 + 300, HEIGHT/4 + 130, 20, BLACK);

	DrawText(TextFormat("\t\tInfo"), WIDTH/4 + 50, HEIGHT/4 + 160, 20, BLACK);
	DrawText("<Ctrl + I>", WIDTH/4 + 300, HEIGHT/4 + 160, 20, BLACK);
	
	DrawText(TextFormat("\t\tExit program"), WIDTH/4 + 50, HEIGHT/4 + 190, 20, BLACK);
	DrawText("<ESC>", WIDTH/4 + 320, HEIGHT/4 + 190, 20, BLACK);
	
	DrawText(TextFormat("\t\tShow HELP"), WIDTH/4 + 50, HEIGHT/4 + 220, 20, BLACK);
	DrawText("<Ctrl + H>", WIDTH/4 + 300, HEIGHT/4 + 220, 20, BLACK);

	DrawText(TextFormat("\t\tImport model"), WIDTH/4 + 50, HEIGHT/4 + 250, 20, BLACK);
	DrawText("<Ctrl + U>", WIDTH/4 + 300, HEIGHT/4 + 250, 20, BLACK);

	DrawText(TextFormat("\t\tRun simulation"), WIDTH/4 + 50, HEIGHT/4 + 280, 20, BLACK);
	DrawText("<Ctrl + T>", WIDTH/4 + 300, HEIGHT/4 + 280, 20, BLACK);

	// column 2
	DrawText("Visual:", WIDTH/4 + 450, HEIGHT/4 + 70, 20, BLACK);

	DrawText(TextFormat("\t\tModel\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 100, 20, BLACK);
	DrawText("<Alt + M>", WIDTH/4 + 750, HEIGHT/4 + 800, 20, BLACK);

	DrawText(TextFormat("\t\tBox\t\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 130, 20, BLACK);
	DrawText("<Alt + B>", WIDTH/4 + 750, HEIGHT/4 + 130, 20, BLACK);

	DrawText(TextFormat("\t\tRadar show/hide"), WIDTH/4 + 450, HEIGHT/4 + 160, 20, BLACK);
	DrawText("<Alt + R>", WIDTH/4 + 750, HEIGHT/4 + 160, 20, BLACK);
	
	DrawText(TextFormat("\t\tGrid\t\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 190, 20, BLACK);
	DrawText("<Alt + G>", WIDTH/4 + 750, HEIGHT/4 + 190, 20, BLACK);
	
	DrawText(TextFormat("\t\tGizmo\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 220, 20, BLACK);
	DrawText("<Alt + J>", WIDTH/4 + 750, HEIGHT/4 + 220, 20, BLACK);

	DrawText(TextFormat("\t\tFPS\t\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 250, 20, BLACK);
	DrawText("<Alt + C>", WIDTH/4 + 750, HEIGHT/4 + 250, 20, BLACK);
	
	DrawText(TextFormat("\t\tPlots\tshow/hide"), WIDTH/4 + 450, HEIGHT/4 + 280, 20, BLACK);
	DrawText("<Alt + P>", WIDTH/4 + 750, HEIGHT/4 + 280, 20, BLACK);

	// link
	DrawText("For more instructions check the link ------->", WIDTH/4 + 10, HEIGHT/4 + 340, 20, BLACK);
	int link = GuiButton((Rectangle) {WIDTH/4 + 500, HEIGHT/4 + 330, 150, 40}, GuiIconText(ICON_NOTEBOOK, "Manual link"));
	...
	switch(gui){
		case 1:
			...
			for(uint16_t i = 0; i < items; ++i){
				int j = 1;
				while(data->point.x[i] < data->point.x[i-j]){
					float buffer_x = data->point.x[i-j];
					float buffer_y = data->point.y[i-j];

					data->point.x[i-j] = data->point.x[i-j+1];
					data->point.y[i-j] = data->point.y[i-j+1];

					data->point.x[i-j+1] = buffer_x;
					data->point.y[i-j+1] = buffer_y;
					j += 1;
				}
			}
			for(uint16_t i = 0; i < items; ++i){
...
int Radar_Group(Radar *radar, Geometry *geometry){
	...
	if((sqrt(radar->x*radar->x+radar->y*radar->y+radar->z*radar->z) < 30) || (radar->distance < 30)) DrawText("The field is NEAR!", WIDTH - (geometry->group_width + geometry->panel_width)/2.4, 195, 20, RED);
	else DrawText("The field is FAR", WIDTH - (geometry->group_width + geometry->panel_width)/2.4, 195, 20, GREEN);
...
int progressWindow(bool *status, float *progress){
	...
	*status = 1;
	...
	DrawText(TextFormat("%d%s", (int) *progress, "%"), WIDTH/8 - 40 + (MeasureText("100%",15) + WIDTH/4)/2, HEIGHT/8 + 170 + 10, 20, BLACK);
